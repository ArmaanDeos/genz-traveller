...
const MobilePromo = ({
  ...
  fixedCtaBgColor = "bg-red-600",
  buttonBgColor = "bg-red-600",
  mainSectionBgColor = "bg-white",
  buttonLabels = [
    "New Bookings",
    "Changes",
    "Cancellations",
  ...
  bannerImage = "/images/Generic-Banner.png",
  planeImage = "/images/airline-generic.png",
  agentImage = "/images/calling-cus.png",
  offersText = {
    domestic: 60,
    international: 50,
  },
  ...
  return (
    <>
      {/* Fixed Top CTA */}
      <div
        className={`${fixedCtaBgColor} fixed top-0 left-0 right-0 w-full py-3 px-4 z-[999999] shadow-md md:hidden`}
      >
        <div className="flex items-center justify-center gap-3 max-w-md mx-auto">
          ...
          <div className="text-center text-white font-medium">
            ...
            <a
              ...
              className="block font-bold text-3xl text-white"
            >
              {phoneNumber}
            </a>
          </div>
      ...
      {/* Main Mobile Section */}
      <section
        className={`${mainSectionBgColor} block md:hidden py-4 px-3 shadow-md rounded mt-[80px] mb-[80px]`}
      >
        {/* Header */}
        <div className={`${fixedCtaBgColor} text-white py-2 rounded mb-4`}>
          <h3 className="text-lg font-semibold text-center">
            {airlineName} Reservations Desk
          </h3>
        </div>
        ...
        <div className="grid grid-cols-2 gap-2 text-sm font-fira mb-4">
          {buttonLabels.map((text, i) => (
            <a
              key={i}
              href={`tel:${phoneNumber}`}
              className={`${buttonBgColor} text-white py-2 rounded text-center`}
            >
              {text}
            </a>
          ))}
        ...
        {/* Phone Box */}
        <div
          className={`${fixedCtaBgColor} text-white rounded-full px-5 py-3 flex flex-wrap items-center justify-center gap-3 mt-4 max-w-xs mx-auto`}
        >
        ...
        <p className="text-center text-gray-800 font-semibold text-base mt-4 px-4">
          Call Our {airlineName} Customer Service&apos;s and Save Up to{" "}
          {offersText.domestic}% on Domestic and Up to{" "}
          {offersText.international}% on International Flight Booking.
        </p>
      </section>
